...
namespace MemoryTraceAndOptimize
{
	class Program
	{
		...
		{
			var orderResultSpan = orderResult.AsSpan();
			var splitEnumerator = orderResultSpan.SplitString(",");
			splitEnumerator.MoveNext();
			//var isSuccess = splitEnumerator.Current.ToString();
			splitEnumerator.MoveNext();
			var orderId = splitEnumerator.Current;
			//splitEnumerator.MoveNext();
			//var cost = splitEnumerator.Current.ToString();
			//splitEnumerator.MoveNext();
			//var bonus = splitEnumerator.Current.ToString();
			return orderId;
		}
	}
